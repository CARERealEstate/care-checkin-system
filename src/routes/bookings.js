...
router.get('/', (req, res) => {
  try {
    ...
    // Count forms and evidence for each booking
    bookings = bookings.map(b => {
      ...
      return {
        ...b,
        has_checkin: ciCount?.c || 0,
        has_checkout: coCount?.c || 0,
        evidence_count: evCount?.c || 0
      };
    });
  ...
  try {
    ...
    const result = db.prepare(`
      INSERT INTO bookings (sangam_id, tenant_first_name, tenant_last_name, tenant_email, tenant_phone,
        property_address, council_name, placement_start, placement_end, reference_number, risk_profile, nightly_rate)
      VALUES (@sangam_id, @first, @last, @email, @phone, @property, @council, @start, @end, @ref, @risk, @rate)
    `).run({
      sangam_id: `manual-${Date.now()}`,
      first: b.tenant_first_name || '',
      last: b.tenant_last_name || '',
      email: b.tenant_email || '',
      phone: b.tenant_phone || '',
      property: b.property_address || '',
      council: b.council_name || '',
      start: b.placement_start || '',
      end: b.placement_end || '',
...
router.put('/:id', (req, res) => {
  try {
    ...
    // Update each field individually (simpler for sql.js)
    const allowed = ['tenant_first_name', 'tenant_last_name', 'tenant_email', 'tenant_phone',
      'property_address', 'council_name', 'placement_start', 'placement_end', 'reference_number', 'status'];
